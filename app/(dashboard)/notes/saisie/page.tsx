...
import type { ParsedData, FileNode } from '../../../../components/notes/types';
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (currentFileNode.handle) {
                ...
                // --- Sync to Database ---
                try {
                    // Reconstruct note list for the API
                    const noteelevList: any[] = [];
                    data.students.forEach((student) => {
                        student.originalEntries.forEach((entry) => {
                            const gradeKey = `${entry.CODETYPEEPRE}_${entry.NUMEEPRE}`;
                            noteelevList.push({
                                ...entry,
                                NOTEEPRE: student.grades[gradeKey] || entry.NOTEEPRE,
                                obseprof: student.observation || "",
                            });
                        });
                    });
                    ...
                    // We don't block the UI if sync fails, but we might want to log it
                }
                // -----------------------
